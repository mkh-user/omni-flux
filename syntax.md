...
OmniFlux supports single-line comments with `#` character, inline comments can be used in same way:
...
exclude test_module # "./test_module.omni" will not be imported anyway.

import * # Everything in current folder
import "D:/Codes/personal_utils_v1" as PUtils # Imports personal_utils_v1.omni, ".omni" is optional

from os import process, path

# config.omni exsists in current folder.
if config.enable_debugger:
	...
	import "../debugger/light_debbuger" # Just imports "light_debugger.omni" module.
```
